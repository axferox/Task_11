...
function switcher() {
  ...
  fs.unlink("./test/config/browser.config.json", (error) => {
    if (error) throw error;
    console.error("./test/config/browser.config.json was not created or deleted already");
  });
  ...
  if (config.browserName == "chrome") {
    ...
    return new Builder()
      ...
      .usingServer(`http://${config.environment}:4444/`)
      .build();
